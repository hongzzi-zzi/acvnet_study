...
class ACVNet(nn.Module):
    ...
    def forward(self, left, right):
        ...
        if self.freeze_attn_weights:
            ...
            features_left = self.feature_extraction(left) # dict_keys(['gwc_feature'])
            features_right = self.feature_extraction(right) # dict_keys(['gwc_feature'])

            # print(self.maxdisp // 4)#13->16으로 바꿔주어야한다궁
            gwc_volume = build_gwc_volume(features_left["gwc_feature"], features_right["gwc_feature"], self.maxdisp // 4, self.num_groups)
            # print(gwc_volume.shape)# torch.Size([1, 40, 13, 264, 192])
            gwc_volume = self.patch(gwc_volume)
            # print(gwc_volume.shape)# torch.Size([1, 40, 13, 264, 192])
            ...
            patch_l2 = self.patch_l2(gwc_volume[:, 8:24])
            patch_l3 = self.patch_l3(gwc_volume[:, 24:40])
            # torch.Size([1, 8, 13, 264, 192])
            # torch.Size([1, 16, 13, 264, 192])
            # torch.Size([1, 16, 13, 264, 192])

            patch_volume = torch.cat((patch_l1,patch_l2,patch_l3), dim=1)
            # print(patch_volume.shape)# torch.Size([1, 40, 13, 264, 192])
            cost_attention = self.dres1_att(patch_volume)
            # print(cost_attention.shape)# torch.Size([1, 16, 13, 264, 192])->13 이 16이; 되야댐
            cost_attention = self.dres2_att(cost_attention)
            att_weights = self.classif_att(cost_attention)

        if not self.attn_weights_only:# 여기 ㄴㄴ
