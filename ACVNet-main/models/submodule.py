...
class attention_block(nn.Module):
    ...
    def forward(self, x):
        B, C, D, H0, W0 = x.shape # torch.Size([1, 64, 7, 66, 48])
        pad_l = pad_t = 0
        pad_r = (self.block[2] - W0 % self.block[2]) % self.block[2] # 0
        pad_b = (self.block[1] - H0 % self.block[1]) % self.block[1] # 2
        x = F.pad(x, (pad_l, pad_r, pad_t, pad_b))
        B, C, D, H, W = x.shape
        d, h, w = D//self.block[0], H//self.block[1], W//self.block[2]
        dd=x.view(B, C, d,self.block[0], h, self.block[1], w, self.block[2])
        x = dd.permute(0, 2, 4, 6, 3, 5, 7, 1)
